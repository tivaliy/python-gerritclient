...
class TestUtils(oslo_base.BaseTestCase):
    def test_get_display_data_single(self):
        ...
        self.assertEqual(
            utils.get_display_data_single(fields=columns, data=data), [1, "test_name"]
        )
    ...
    def test_get_display_data_multi_w_sorting(self):
        ...
        # by single field
        self.assertEqual(
            utils.get_display_data_multi(fields=columns, data=data, sort_by=["name"]),
            [
                [2, "amazon", "error"],
                [17, "facebook", "note"],
                [15, "google", "warning"],
    ...
    def test_normalize(self):
        self.assertEqual(utils.normalize("#/foo+bar_$!str.", ""), "foobarstr.")
