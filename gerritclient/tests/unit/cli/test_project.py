...
class TestProjectCommand(clibase.BaseCLITest):
    ...
    def test_project_list_limit(self):
        list_limit = 5
        args = f"project list --limit {list_limit}"
        self.exec_command(args)
    ...
    def test_project_list_skip_first(self):
        list_skip = 5
        args = f"project list --skip {list_skip}"
        self.exec_command(args)
    ...
    def test_project_list_range(self):
        list_skip = 2
        list_limit = 2
        args = f"project list --skip {list_skip} --limit {list_limit}"
        self.exec_command(args)
    ...
    def test_project_list_w_specified_type(self):
        prj_type = "code"
        args = f"project list --type {prj_type}"
        self.exec_command(args)
    ...
    def test_project_show(self):
        ...
        args = f"project show {project_id}"
        self.exec_command(args)
    ...
    def test_project_create_w_default_parameters(self):
        ...
        args = f"project create {project_id}"
        self.exec_command(args)
    ...
    def test_project_delete(self):
        ...
        args = f"project delete {project_name}"
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.delete.assert_called_once_with(
            project_name, force=False, preserve=False
        )

    def test_project_delete_w_force(self):
        project_name = "fake/fake-project"
        args = f"project delete {project_name} --force"
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.delete.assert_called_once_with(
            project_name, force=True, preserve=False
        )

    def test_project_delete_w_preserve_git_repository(self):
        project_name = "fake/fake-project"
        args = f"project delete {project_name} --preserve-git-repository"
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.delete.assert_called_once_with(
            project_name, force=False, preserve=True
        )

    def test_project_description_show(self):
        project_name = "fake/fake-project"
        args = f"project description show {project_name}"
    ...
    def test_project_description_set(self):
        ...
        args = f'project description set {project_name} --description "{project_description}" --message "{message}"'
        self.m_client.set_description.return_value = project_description
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.set_description.assert_called_once_with(
            project_name, description=project_description, commit_message=message
        )

    def test_project_parent_show(self):
        project_name = "fake/fake-project"
        args = f"project parent show {project_name}"
    ...
    def test_project_parent_set(self):
        ...
        message = "Fake commit message"
        args = f'project parent set {project_name} --parent "{parent_project}" --message "{message}"'
    ...
    def test_project_head_show(self):
        project_name = "fake/fake-project"
        args = f"project head show {project_name}"
    ...
    def test_project_head_set(self):
        project_name = "fake/fake-project"
        branch = "fake-branch"
        args = f"project head set {project_name} --branch {branch}"
        self.m_client.set_head.return_value = f"refs/heads/{branch}"
        self.exec_command(args)
    ...
    def test_project_repo_statistics_show(self):
        project_name = "fake/fake-project"
        args = f"project repo-statistics show {project_name}"
    ...
    def test_project_branch_list(self):
        project_name = "fake/fake-project"
        args = f"project branch list {project_name}"
    ...
    def test_project_branch_show(self):
        ...
        args = f"project branch show {project_name} --branch {branch_name}"
    ...
    def test_project_branch_create(self):
        ...
        args = f"project branch create {project_name} --branch {branch_name} --revision {revision}"
        fake_branch = fake_project.get_fake_project_branch(
            ref=branch_name, revision=revision
        )
    ...
    def test_project_branch_reflog_show(self):
        project_name = "fake/fake-project"
        branch = "fake-branch"
        args = f"project branch reflog show {project_name} --branch {branch}"
    ...
    def test_project_child_list(self):
        project_name = "fake/fake-project"
        args = f"project child list {project_name}"
        fake_projects = [fake_project.get_fake_project() for _ in range(5)]
        self.m_client.get_children.return_value = fake_projects
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.get_children.assert_called_once_with(
            project_name, recursively=False
        )

    def test_project_child_list_recursively(self):
        project_name = "fake/fake-project"
        args = f"project child list {project_name} --recursively"
        fake_projects = [fake_project.get_fake_project() for _ in range(5)]
        fake_projects.append(
            fake_project.get_fake_project(
                project_id="child-project", name="child-project", parent=project_name
            )
        )
        self.m_client.get_children.return_value = fake_projects
        self.exec_command(args)
    ...
    def test_project_run_garbage_collection_wo_parameters(self):
        ...
        args = f"project gc-run {project_name}"
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.run_gc.assert_called_once_with(
            project_name, aggressive=False, show_progress=False
        )
...
Garbage collection completed successfully."""
        self.m_client.run_gc.return_value = msg
        args = f"project gc-run {project_name} --aggressive --show-progress"
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.run_gc.assert_called_once_with(
            project_name, aggressive=True, show_progress=True
        )

    def test_project_tag_list(self):
        project_name = "fake/fake-project"
        args = f"project tag list {project_name}"
        self.m_client.get_tags.return_value = fake_tag.get_fake_tags(5)
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.get_tags.assert_called_once_with(
            project_name, limit=None, skip=None, pattern_dispatcher=None
        )

    def test_project_tag_list_limit(self):
        project_name = "fake/fake-project"
        list_limit = 5
        args = f"project tag list {project_name} --limit {list_limit}"
        self.m_client.get_tags.return_value = fake_tag.get_fake_tags(10)
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.get_tags.assert_called_once_with(
            project_name, limit=list_limit, skip=None, pattern_dispatcher=None
        )

    def test_project_tag_list_skip_first(self):
        project_name = "fake/fake-project"
        list_skip = 5
        args = f"project tag list {project_name} --skip {list_skip}"
        self.m_client.get_tags.return_value = fake_tag.get_fake_tags(10)
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.get_tags.assert_called_once_with(
            project_name, limit=None, skip=list_skip, pattern_dispatcher=None
        )
    ...
    def test_project_tag_show(self):
        ...
        args = f"project tag show {project_name} {tag_id}"
    ...
    def test_project_tag_create(self):
        ...
        args = f"project tag create {project_name} --tag {tag_id} -r {revision} -m '{message}'"
        self.m_client.get_tag.return_value = fake_tag.get_fake_tag()
        self.exec_command(args)

        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.create_tag.assert_called_once_with(
            project_name, tag_id, revision=revision, message=message
        )
    ...
    def test_project_configuration_download_json(self, m_dump):
        ...
        args = f"project configuration download {project_name} -f {file_format} -d {directory}"
        expected_path = f"{directory}/{utils.normalize(project_name)}.{file_format}"

        self.m_client.get_config.return_value = test_data

        m_open = mock.mock_open()
        with mock.patch("gerritclient.commands.project.open", m_open, create=True):
            self.exec_command(args)
    ...
    def test_project_commit_show(self):
        ...
        project_name = "fake/fake-project"
        args = f"project commit show {project_name} --commit {commit_id}"
        ...
        self.m_get_client.assert_called_once_with("project", mock.ANY)
        self.m_client.get_commit.assert_called_once_with(project_name, commit_id)
    ...
    def test_project_commit_affiliation_show(self):
        commit_id = "184ebe53805e102605d11f6b143486d15c23a09c"
        project_name = "fake/fake-project"
        args = f"project commit included-in {project_name} --commit {commit_id}"
    ...
    def test_project_commit_file_content_show(self):
        ...
        args = f"project commit file-content show {project_name} --commit {commit_id} --file-id {file_id}"
