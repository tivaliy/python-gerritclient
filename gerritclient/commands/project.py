...
class ProjectList(ProjectMixIn, base.BaseListCommand):
    ...
    columns = ("name", "id", "state", "web_links")
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            "--skip",
            type=int,
            help="Skip the given number of projects from the beginning of the list.",
        )
        parser.add_argument(
            "--type",
        ...
        group.add_argument(
            "-r",
            "--regex",
            help="Limit the results to those projects that match the specified regex.",
        )
        return parser
...
class ProjectShow(ProjectMixIn, base.BaseShowCommand):
    ...
    columns = ("id", "name", "parent", "description", "state", "web_links")
...
class ProjectDelete(ProjectMixIn, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        ...
        msg = f"Project '{parsed_args.name}' was deleted\n"
...
class ProjectDescriptionShow(ProjectMixIn, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        ...
        self.app.stdout.write(f"{response}\n")
...
class ProjectParentSet(ProjectMixIn, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        response = self.client.set_parent(
            parsed_args.name,
            parent=parsed_args.parent,
            commit_message=parsed_args.message,
        )
        self.app.stdout.write(
            f"A new parent project '{response}' was set for project '{parsed_args.name}'.\n"
        )
...
class ProjectHeadSet(ProjectMixIn, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        response = self.client.set_head(parsed_args.name, branch=parsed_args.branch)
        msg = f"HEAD for the project '{parsed_args.name}' was set to the branch '{response}'\n"
        self.app.stdout.write(msg)
...
class ProjectBranchShow(ProjectMixIn, base.BaseShowCommand):
    ...
    def get_parser(self, app_name):
        ...
        parser.add_argument(
            ...
            "--branch",
            required=True,
            help="The name of a branch or HEAD. The prefix refs/heads/ can be omitted.",
        )
        return parser

    def take_action(self, parsed_args):
        response = self.client.get_branch(
            parsed_args.entity_id, branch_name=parsed_args.branch
        )
        data = utils.get_display_data_single(self.columns, response)
        return self.columns, data
...
class ProjectBranchReflogShow(ProjectMixIn, base.BaseListCommand):
    ...
    def take_action(self, parsed_args):
        response = self.client.get_reflog(parsed_args.name, branch=parsed_args.branch)
        data = utils.get_display_data_multi(self.columns, response)
        return self.columns, data
...
class ProjectTagList(ProjectMixIn, base.BaseListCommand):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            ...
            "--skip",
            type=int,
            help="Skip the given number of tags from the beginning of the list.",
        )
        group = parser.add_mutually_exclusive_group()
        group.add_argument(
            "-m",
    ...
    def take_action(self, parsed_args):
        fetched_pattern = {
            k: v
            for k, v in (("match", parsed_args.match), ("regex", parsed_args.regex))
            if v is not None
        } or None
...
class ProjectConfigDownload(ProjectMixIn, base.BaseDownloadCommand):
    ...
    def take_action(self, parsed_args):
        file_name = f"{utils.normalize(parsed_args.name)}.{parsed_args.format}"
        file_path = os.path.join(os.path.abspath(parsed_args.directory), file_name)
        ...
        try:
            ...
            msg = f"Could not store {self.entity_name} data at {file_path}. {e}"
            raise error.InvalidFileException(msg)
        msg = f"Information about the {self.entity_name} was stored in '{file_path}' file.\n"
        self.app.stdout.write(msg)
...
class ProjectConfigSet(ProjectMixIn, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        ...
        try:
            ...
            msg = (
                "Could not read configuration metadata for the project "
                f"'{parsed_args.name}' at {file_path}"
            )
            raise error.InvalidFileException(msg)

        self.client.set_config(parsed_args.name, data=data)
        msg = f"Configuration of the project '{parsed_args.name}' was successfully updated.\n"
        self.app.stdout.write(msg)
...
class ProjectCommitShow(ProjectMixIn, base.BaseShowCommand):
    ...
    def take_action(self, parsed_args):
        response = self.client.get_commit(parsed_args.entity_id, parsed_args.commit)
        data = utils.get_display_data_single(self.columns, response)
        return self.columns, data
...
class ProjectCommitIncludedIn(ProjectMixIn, base.BaseShowCommand):
    ...
    def take_action(self, parsed_args):
        response = self.client.get_commit_affiliation(
...
class ProjectCommitFileContentShow(ProjectMixIn, base.BaseCommand):
    ...
    def take_action(self, parsed_args):
        response = self.client.get_file_content(
            parsed_args.name, parsed_args.commit, parsed_args.file_id
        )
        self.app.stdout.write(response)
